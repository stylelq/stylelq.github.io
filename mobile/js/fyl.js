...
var kangjaewon = "강재원은 미래의 조각에 관심을 가지고 3D 모델링 소<br>프트웨어를 활용해 조각을 창작합니다. 디지털 환경<br>과 소프트웨어의 기능에서 발생하는 감각을 토대로<br>" + "자신만의 조각을 만들어 왔습니다. 완성된 조각은 디<br>지털 이미지, 영상, 3D프린팅, 알루미늄, 스테인레스,<br>인플레이터블 등 다양한 매체로 변주됩니다.<br><br>" + "강재원은 3D 모델링 프로그램 속 편의를 위해 제공되<br>는 기능이 역으로 현실의 조각을 구성하는 원리에 영<br>향을 미치는 점에 주목했습니다. 전통적인 조각 방식<br>" + "과 달리 작가는 화면 너머로 다양한 각도로 물체를 돌려가며 ‘Move’로 당기고 ‘Smooth’로 밀며 형태를 만<br>들어나갑니다. <span>Deformation 20, 21, 22</span>는 기본 입<br>" + "체 도형인 3개의 실린더 기둥을 3D 프로그램 내부의<br>현실의 물리법칙을 모사한 기능들(Deformation:<br>Skew, Twist, Bend, Gravity 등)을 적용해 변주하는 방<br>" + "식을 추가했습니다. 조절바(control panel)의 수치로<br>형태를 조절하고 ‘ctrl+z’와 ‘ctrt+shift+z’를 적용해 과<br>거와 현재를 오가며 작가가 원하는 조형성을 갖춘 형<br>" + "태를 선택합니다. 순서와 강도의 조합에 따라 전혀 다<br>른 결과가 도출되기에 신중하게 블렌딩하는 과정을<br>거쳐 완성됩니다. 완성된 조각들은 3D 파일로 존재하<br>며 조각들 주변에 환경과 조명을 설정한 렌더 이미지<br>" + "와 영상, 크롬 도금한 3D 프린팅, 주조된 금속, 인플레<br>이터블(Inflatable) 등 다양한 크기와 소재, 매체를 가<br>로지르며 미래의 조각으로 변화합니다. ";
var isVisible = false;
